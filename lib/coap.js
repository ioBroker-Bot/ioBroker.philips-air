...
// Ceiling for the reconnect backoff. It is a trade-off: a longer ceiling spares the network while a
// device is off, but a device that comes back waits that long to be noticed. Two minutes keeps the
// worst case short; the log stays quiet through the level damping below, not through the delay.
const MAX_RECONNECT_INTERVAL = 120000;
// Number of consecutive failures still reported as an error; everything after that goes to debug.
...
class AirPurifier extends EventEmitter {
    ...
    /**
     * Upper bound for one connect attempt: sync() is capped by aliveTimeout, _subscribeOnStatus() by
     * subscribeTimeout. A safety net shorter than this expires while the attempt is still running.
     *
     * @returns milliseconds an attempt can legitimately take
     */
    _attemptBudget() {
        return this.aliveTimeout + this.subscribeTimeout;
    }

    /**
     * (Re-)arm the retry timer, replacing a pending one.
     *
    ...
    async _reconnect() {
        ...
        // Safety net for an attempt that never settles at all; the regular retry is scheduled in the
        // catch below. It must outlast the attempt itself: with a net of only reconnectInterval (which
        // is >= aliveTimeout by construction) it expires in the very tick in which the attempt fails,
        // starts the next attempt right there and discards the backoff the catch just computed. That
        // is the fixed rhythm #377 is about - measured on an AC2889 (24.08.2026): attempts 1 -> 2 and
        // 3 -> 4 followed after a plain aliveTimeout although the log announced 100s resp. 120s.
        this._scheduleRetry(this._attemptBudget() + this.reconnectInterval);
        ...
        try {
            ...
            // sync()/subscribe may reject (e.g. device offline). The retry is (re-)scheduled below, so
            // just surface the error here instead of producing an unhandled rejection.
            this._failedAttempts = (this._failedAttempts || 0) + 1;
            // Back off instead of hammering the device in a fixed rhythm: a switched-off device would
            // otherwise produce an error line every reconnectInterval for as long as it stays off (#377).
            const delay = Math.min(this.reconnectInterval * 2 ** (this._failedAttempts - 1), MAX_RECONNECT_INTERVAL);
            this._scheduleRetry(delay);
            // The first attempts are worth an error line; a device that stays off must not fill the log
            // forever, so keep the repetitions in the debug log. Recovery is visible as "Connected to ...".
            this.emit(
                this._failedAttempts <= LOUD_ATTEMPTS ? 'error' : 'debug',
                `Connection to ${this.deviceIp} (CoAP) failed (attempt ${this._failedAttempts}), retry in ${Math.round(delay / 1000)}s: ${err && err.message ? err.message : err}`,
            );
            ...
            if (this._failedAttempts === LOUD_ATTEMPTS) {
                this.emit(
                    'info',
    ...
    /**
     * Watchdog for the push-based observe stream. Silence alone is NOT proof that the session is gone:
     * a CX7550/01 sent a single status frame in 4.6 hours while answering all 26 /sys/dev/sync probes
     * in the same log (#377). Rebuilding the session on silence therefore tore down a perfectly working
     * connection every staleTimeout, forever.
     *
     * So ask the device first, then renew the subscription either way:
     *
     * - probe fails  -> full reconnect, exactly what used to happen unconditionally. Firmwares that
     *   never answer /sys/dev/sync (e.g. AC2889 fw 1.0.7) therefore behave as before and cannot flap.
     * - probe succeeds -> re-subscribe quietly: the observe registration is renewed just like a
     *   reconnect would have done, but the connection is never marked lost and nothing reaches the
     *   info log. This matters for a device whose observe registration silently expired: keeping the
     *   old request open would mean it never receives data again.
     *
     * The probe costs one round trip per staleTimeout and refreshes the command key on the way; a
     * failing one delays the reconnect by at most aliveTimeout.
     */
    async _checkAlive() {
        if (this.destroyed) {
            return;
        ...
        this.emit(
            'debug',
            `No status from ${this.deviceIp} (CoAP) for ${Math.round(this.staleTimeout / 1000)}s, probing the device...`,
        );
        try {
            await this.sync();
        } catch (err) {
            this.emit(
                'debug',
                `${this.deviceIp} did not answer the probe (${err && err.message ? err.message : err}), reconnecting...`,
            );
            this._reconnect();
            return;
        }
        ...
        this.emit('debug', `${this.deviceIp} answered the probe, renewing the status subscription`);
        // _markConnected() emits nothing while the session is already marked connected, so a quiet but
        // healthy device produces no info line at all - only the watchdog is re-armed.
        this._markConnected(`Connected to ${this.deviceIp} using CoAP`);
        try {
            await this._subscribeOnStatus();
        } catch (err) {
            this.emit(
                'debug',
                `Could not renew the status subscription for ${this.deviceIp} (${err && err.message ? err.message : err}), reconnecting...`,
            );
            this._reconnect();
        }
    }
                ...
                res.on('data', chunk => {
                    ...
                    decryptPromise
                        .then(status => {
                            ...
                            let parsed;
                            // The friendly names renameAttributes() actually assigned, handed to the
                            // listener so it can tell them from raw keys that merely collide.
                            let renamed;
                            try {
                                parsed = JSON.parse(status);
                                renamed = this.renameAttributes(parsed);
                            } catch (err) {
                                this.emit(
                                    'error',
                            ...
                            if (parsed && parsed.state && parsed.state.reported) {
                                this.emit('status', parsed.state.reported, renamed);
                            }
                        })
                        .catch(err => {
                            this.emit(
     ...
     * @returns the friendly names that were assigned (see mapping.renameReported)
     */
    renameAttributes(status) {
        if (status && status.state) {
            return this.renameReported(status.state.reported);
        }
        return new Set();
    }
}
