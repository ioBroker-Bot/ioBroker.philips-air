...
const STANDARD_MAPPING = {
    ...
    // Device-info strings sent by classic devices (AC4236/14 log, GitHub #150). `language` is the
    // display language of the appliance itself, `DeviceVersion` its hardware/firmware revision -
    // neither is controllable, both are plain info that belongs in device.* rather than
    // unknownStates.*.
    language: { name: 'language', device: true, type: 'string', role: 'text' },
    DeviceVersion: { name: 'deviceVersion', device: true, type: 'string', role: 'text' },
    ...
    // Total filter life belonging to the fltsts* remaining hours above; the classic counterpart of
    // the AC3221's D05207/D05408. Confirmed twice over on the AC4236/14 log (GitHub #150): the pairs
    // read 403/720, 1248/4800 and 65535/65535, and kongo09's const.py names the same three keys
    // FILTER_PRE_TOTAL / FILTER_HEPA_TOTAL / FILTER_ACTIVE_CARBON_TOTAL. The friendly names are
    // deliberately shared with the AC3221 entries - a device only ever sends one of the two
    // spellings, and updateStatus() guards against writing a name twice per frame.
    flttotal0: { name: 'preFilterTotalHours', filter: true, unit: 'hours' },
    flttotal1: { name: 'hepaFilterTotalHours', filter: true, unit: 'hours' },
    flttotal2: { name: 'activeCarbonFilterTotalHours', filter: true, unit: 'hours' },
...
const MODEL_MAPPING = {
    AC2889: {
        ...
        om: {
            name: 'fanSpeed',
            // 'as' is the fan speed that goes with the 'allergenSleep' mode below - both are
            // reported and written as a pair. Same source as the mode entry (GitHub #150).
            options: { s: 'silent', as: 'allergenSleep', t: 'turbo', a: 'auto', 1: '1', 2: '2', 3: '3' },
            control: true,
            role: 'level.speed',
        },
        ...
        mode: {
            ...
            options: {
                ...
                AG: 'automode',
                GT: 'gentle',
                // The combined allergen + sleep preset ("Allergie-/Ruhemodus" on the German UI).
                // Reported by the AC4236/14 and missing here since the first report (GitHub #150);
                // kongo09's PhilipsAC4236 class lists the same pair (mode 'AS' with speed 'as').
                AS: 'allergenSleep',
            },
            control: true,
        },
...
function createMapping(model) {
     ...
     * @returns the friendly names this call assigned - the only reliable way to tell a mapped
     *          attribute from a raw key that merely collides with a friendly name
     */
    function renameReported(reported) {
        // The friendly names this call actually produced. A name alone does not prove a mapping ran:
        // a raw key can be spelled exactly like some model's friendly name (the classic `mode` key vs.
        // the AC3221 control D0310C -> `mode`), so callers must be able to tell a renamed attribute
        // from an untouched raw one that happens to collide. Returned instead of derived afterwards
        // because only this loop knows which keys it claimed.
